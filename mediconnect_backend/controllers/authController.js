...
const db = require('../models'); // Access the database
...
exports.register = async (req, res) => {
  try {
    ...
    // 3. Set Status (Patients = Active, Staff = Pending)
    ...
    if (role === 'doctor') {
      await DoctorDetails.create({ userId: newUser.id, specialization: specialization || 'General' });
    } else if (role === 'nurse') {
      await NurseDetails.create({ userId: newUser.id, department: department || 'General' });
    }
